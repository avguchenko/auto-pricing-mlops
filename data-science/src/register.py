...
def main(args):
    ...
    # Step 2: log the loaded model under the given name
    mlflow.sklearn.log_model(model, args.model_name)

    # Step 3: register the logged model and capture its version
    run_id = mlflow.active_run().info.run_id
    model_uri = f"runs:/{run_id}/{args.model_name}"
    mlflow_model = mlflow.register_model(model_uri, args.model_name)
    model_version = mlflow_model.version

    # Step 4: write registration details to a JSON file
